...
import { getCurrentWorkout, getExercisesByWorkoutId, getLogs, getTimeFinished, getTimeStarted } from "@/app/actions";
...
async function FetchData({ workoutId }: { workoutId: number }) {
    const [currentWorkout, exercises, logs, started, finished] = await Promise.all([
        getCurrentWorkout(workoutId),
        getExercisesByWorkoutId(workoutId),
        getLogs(),
        getTimeStarted(workoutId),
        getTimeFinished(workoutId)
    ]);
    ...
    return <Workout
        ...
        initialLogs={logs}
        initialStarted={started ? parseInt(started, 10) : null}
        initialFinished={finished ? parseInt(finished, 10) : null}
    />;
}
